...
import { Button, Modal } from '@bunnymoon-libs/uikit'
  ...
  return (
    <Modal title={t('Enter amount of tickets to buy')} onDismiss={onDismiss}>
      <TicketInput
        ...
        max={fullBalance}
        symbol="TICKET"
        availableSymbol="BUNNYMOON"
      />
      <div>
        <Tips>{t(`1 Ticket = ${LOTTERY_TICKET_PRICE} BUNNYMOON`, { num: LOTTERY_TICKET_PRICE })}</Tips>
      </div>
      <div>
        <Announce>
          {t('Ticket purchases are final. Your BUNNYMOON cannot be returned to you after buying tickets.')}
        </Announce>
        <Final>{t('You will spend: %amount% BUNNYMOON', { amount: cakeCosts(val) })}</Final>
      </div>
      <ModalActions>
